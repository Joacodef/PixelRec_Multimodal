# scripts/evaluate.py - Updated with model-specific checkpoint loading and robust scaler handling
#!/usr/bin/env python
"""
...
# Import the processor to load the scaler correctly
...
def find_model_checkpoint(config: Config, checkpoint_name: str = 'best_model.pth') -> Path:
    ...
    alternative_names = ['final_model.pth', 'last_model.pth', 'model.pth', 'checkpoint.pth']
...
def main():
    ...
    if config_obj.data.numerical_normalization_method in ['standardization', 'min_max']:
        ...
        if scaler_path.exists():
            ...
            if num_processor.load_scaler(scaler_path) and num_processor.fitted_columns:
                ...
                # USE THE COLUMNS FROM THE SCALER, NOT THE CONFIG
                numerical_features_for_dataset = num_processor.fitted_columns
                numerical_scaler = num_processor.scaler
            elif num_processor.scaler:
                 print(f"  → Warning: Loaded scaler from {scaler_path} but it has no column info. Using columns from config file.")
                 numerical_scaler = num_processor.scaler
        else:
    ...
    dataset_for_encoders = MultimodalDataset(
        ...
        # Pass the robustly determined features and scaler
    ...
    if args.recommender_type == 'multimodal':
        ...
        model_params = {
            ...
            'num_numerical_features': len(numerical_features_for_dataset), # Use the robustly determined count
        ...
        if not hasattr(recommender_instance, 'get_recommendations_parallel'):
            ...
            def parallel_get_recommendations(user_id, top_k=10, filter_seen=True, candidates=None):
                return recommender_instance.get_recommendations_parallel(
                    user_id=user_id, top_k=top_k, filter_seen=filter_seen, candidates=candidates,
                    num_workers=args.num_workers, chunk_size=args.parallel_chunk_size
                )
    ...
    results['evaluation_metadata'] = {
        'task': evaluator.task_name, 'filter_seen_evaluator_perspective': evaluator.filter_seen,
        'recommender_type': args.recommender_type, 'top_k': config_obj.recommendation.top_k, 
        'test_file': args.test_data, 'train_file_used_for_history': args.train_data if args.train_data else "None",
        'cache_warmup_enabled': args.warmup_recommender_cache, 'parallel_evaluation': args.use_parallel,
        ...
        'model_combination': model_combo, 'checkpoint_used': args.checkpoint_name,
